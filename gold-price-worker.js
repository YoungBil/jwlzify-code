// gold-price Cloudflare Worker — live gold spot price proxy (metals.dev).
//
// Contract (what ailab.html + pricing.js consume):
//   GET → JSON { price: <pure 24k gold, USD per GRAM> }
//   The front end multiplies by karat purity (10k ×0.417, 14k ×0.583) itself.
//
// Deploy (Cloudflare dashboard ONLY — never Wrangler CLI):
//   Worker name: gold-price  →  https://gold-price.sarkd333.workers.dev
...
export default {
  async fetch(request, env) {
    ...
    // Fallback: sensible 24k USD/g so pricing still works if metals.dev is down.
    const FALLBACK = 106.0;
    if (!env.METALS_API_KEY) {
      console.error('[gold-price] METALS_API_KEY not set — serving fallback');
    ...
    try {
      ...
      const price = data && data.metals && data.metals.gold;
      if (!(price > 0)) throw new Error('no gold price in response');
      return new Response(JSON.stringify({ price: price }), {
        headers: { ...CORS, 'Content-Type': 'application/json', 'Cache-Control': 'max-age=60' },
      });
    } catch (e) {
      console.warn('[gold-price] upstream failed:', e.message, '— serving fallback');
      return new Response(JSON.stringify({ price: FALLBACK, source: 'fallback' }), {
        headers: { ...CORS, 'Content-Type': 'application/json' },
      });
    }
